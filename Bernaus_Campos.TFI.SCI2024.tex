...
\section{Definición del problema}
	...
	La manera en la que los CPU de las computadoras modernas regulan su temperatura es a través de sensores de temperatura que dan la orden a diferentes sistemas de enfriamiento para mantener la temperatura en un rango deseado. Una de las opciones más populares para enfriar es un ventilador. Mantener las temperaturas bajas es crucial para garantizar el correcto funcionamiento de una computadora, ya que si no, puede llevar a que la computadora se apague al alcanzar los límites superiores para prevenir daños permanentes a sus componentes
	Basado en este concepto, el sistema propuesto es un controlador a lazo cerrado para regular la temperatura de un CPU mediante el ajuste de la velocidad de un ventilador. El principio de funcionamiento se basa en la retroalimentación: se mide constantemente la temperatura del CPU, se compara con un valor deseado (setpoint), y se ajusta dinámicamente la velocidad del ventilador para mantener la temperatura en los límites establecidos.
...
\section{Análisis de la planta}
	 ...
	 \begin{equation}
	 	I[A]=\frac{\Delta V[V]}{R[ohm]}=Q[W]=\frac{\Delta T[Cº]}{Rth[Cº/W]}
	 \end{equation}
	...
	\subsection{Modelado matemático de los componentes}
		...
		Para obtener la función de transferencia del comparador, suponemos una $V_{ref}=600[mV]$. Esta se condice con una temperatura de 60 ºC. La señal de entrada va ser el error que resulta de la resta $V_{ref}-V_{LM35}$; en el caso de que el error sea $0[V]$, la salida de PWM al ventilador es cero. Esto significa que está operando a una temperatura segura. Para el caso donde el error sea $-300[mV]$, es decir que la temperatura del CPU ronda los 90 ºC, la salida PWM debe ser $5[V]$. Entre medio de estos dos valores definimos una variación lineal; para errores positivos la salida de PWM también debe ser $0[V]$ esto significa que esta por debajo de los 60 ºC. Con este análisis, calculamos una recta que pase por estos dos puntos, para luego calcular la función de transferencia.\newpage
	...
	Verificamos el comportamiento del sistema simulando la respuesta al escalón unitario con distintos valores de kp. La forma de respuesta es decreciente: esto se debe a que aumentar la señal de entrada disminuye la salida (ganancia negativa). Este comportamiento es esperable,  ya que la temperatura debe disminuir cuando la señal de control (PWM) aumenta.
...
\section{Diseño del controlador}
	...
	En este gráfico se verifica la cancelación de los dos polos mas dominantes y el polo al origen que se agrega por el PID.
	\begin{figure}[h!]
		\centering
		\includegraphics[width=1\linewidth]{Imagenes/Pzmap}
		\caption[Polos y ceros, antes vs después de la compensación]{Polos y ceros, antes vs después de la compensación}
		\label{fig:pzmap}
	\end{figure}
...
\section{Simulación}
	En esta sección, nos interesa comparar la respuesta temporal antes y después de compensar, además de calcular el error de estado estable del sistema.
